...
netdev_vport_needs_dst_port(const struct netdev *dev)
{
    ...
    return (class->get_config == get_tunnel_config &&
            (!strcmp("vxlan", type) || !strcmp("vxlan-sclp", type) || !strcmp("lisp", type)));
}
                           ...
                           char namebuf[], size_t bufsize)
{
	...
	if (!strcmp(type, "vxlan-sclp")) {
	    snprintf(namebuf, bufsize, "sclp_sys_%d", ntohs(vport->tnl_cfg.dst_port));
	} else {
...
set_tunnel_config(struct netdev *dev_, const struct smap *args)
{
    ...
    if (((!strcmp(type, "vxlan")) || (!strcmp(type, "vxlan-sclp"))) && !tnl_cfg.dst_port) {
...
get_tunnel_config(const struct netdev *dev, struct smap *args)
{
    ...
    if (tnl_cfg.dst_port) {
        ...
        if (((!strcmp("vxlan", type) || !strcmp("vxlan-sclp", type)) && dst_port != VXLAN_DST_PORT) ||
...
netdev_vport_tunnel_register(void)
{
    static const struct vport_class vport_classes[] = {
        ...
        TUNNEL_CLASS("vxlan-sclp", "vxlan_sclp_system"),
