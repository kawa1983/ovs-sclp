...
netdev_vport_needs_dst_port(const struct netdev *dev)
{
    ...
    return (class->get_config == get_tunnel_config &&
            (!strcmp("geneve", type) || !strcmp("vxlan", type) || !strcmp("vxlan-sclp", type) ||
...
netdev_vport_construct(struct netdev *netdev_)
{
    ...
    } else if (!strcmp(type, "vxlan-sclp")) {
        dev->tnl_cfg.dst_port = htons(VXLAN_DST_PORT);
...
set_tunnel_config(struct netdev *dev_, const struct smap *args)
{
    ...
    has_csum = strstr(type, "gre") || strstr(type, "geneve") ||
               strstr(type, "stt") || strstr(type, "vxlan") || strstr(type, "vxlan-sclp");
    ...
    if (!strcmp(type, "vxlan") || !strcmp(type, "vxlan-sclp")) {
    ...
    SMAP_FOR_EACH (node, args) {
        if (!strcmp(node->key, "remote_ip")) {
            ...
            while (ext) {
                if ((!strcmp(type, "vxlan") || !strcmp(type, "vxlan-sclp")) && !strcmp(ext, "gbp")) {
...
get_tunnel_config(const struct netdev *dev, struct smap *args)
{
    ...
    if (tnl_cfg.dst_port) {
        ...
        if ((!strcmp("geneve", type) && dst_port != GENEVE_DST_PORT) ||
            ...
            (!strcmp("vxlan-sclp", type) && dst_port != VXLAN_DST_PORT) ||
...
netdev_vport_tunnel_register(void)
{
    ...
    static const struct vport_class vport_classes[] = {
        ...
        TUNNEL_CLASS("vxlan-sclp", "sclp_sys", netdev_vport_alloc, NULL, NULL, NULL),
